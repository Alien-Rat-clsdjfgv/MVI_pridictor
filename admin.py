    ...
    if df.empty:
        ...
        with col3:
            if "actual_mvi" in df.columns:
                known_mvi = len(df[df["actual_mvi"].notnull()])
        ...
        with col1:
            if "risk_level" in df.columns and not df["risk_level"].empty:
                risk_filter = st.multiselect("Filter by Risk Level", df["risk_level"].unique())
            else:
                risk_filter = []
